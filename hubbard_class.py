...
class hubbard:
    def __init__(
        self, N: int = 2, t=-1, U=4, mu=2, hopping_matrix=np.matrix([[0, 1], [1, 0]])
    ):
        self._N = N
        self._t = t
        self._U = U
                ...
                for spin in ["+", "-"]:
                    if not i == j and not self._hopping_matrix[i, j] == 0:
                        ...
                        if not new_state._void:
                            ...
                            if new_state._state not in [
                                state._state for state in block_states
                            ]:
    ...
    def __calculate_block_from_init_state(self, initial_state: fock_state):
        ...
        new_block = block(
            associations, self._t, self._U, self._mu, self._hopping_matrix
        )
        bisect.insort(
    ...
    def calculate_block(self, N_electrons, spin):
        ...
        for num in self._bank:
            ...
            if (
                initial_state._num_electrons == N_electrons
                and initial_state._total_spin == spin
            ):
                found = True
                break
    ...
    def __repr__(self):
        ...
        if self._gs_energy is not None:
